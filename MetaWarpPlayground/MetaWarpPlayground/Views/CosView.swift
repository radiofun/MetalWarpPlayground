...
struct CosView : View {
    ...
    var body: some View {
        BaseView()
            .ignoresSafeArea()
            .layerEffect(ShaderLibrary.particlecos(.float2(circleSize,circleSize),.float2(dragging),.float(intensity),.float(progress)), maxSampleOffset: CGSize(width: 200, height: 200))
            .gesture(
                DragGesture()
                    .onChanged { value in
